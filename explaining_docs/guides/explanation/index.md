# Explanation

Explanations are **understanding-oriented**. They clarify a particular topic, providing background, context, and the "why" behind the software.

*Current explanations:*
- [Engine Modes Explained](modes.md)
- [Deep Dive: Game Mode Execution Trace](game_mode/execution_trace_turn.md)
- [Deep Dive: Conversation Mode Execution Trace](conversation_mode/execution_trace.md)
- [Deep Dive: Roleplay Mode Execution Trace](roleplay_mode/execution_trace.md)
- [The Expression System (Sprite Swapping & Animation)](expression_system.md)
- [How AI Image Generation works in the Engine](image_generation.md)
- [AI Steering & Guided Generation](ai_steering.md)
- [Asymmetric Mode Linking](mode-linking.md)
- [History, Swipes, and Snapshots](history.md)
- [Understanding Presets: Chat vs Prompt](presets.md)
- *How Context token limits are calculated (Pending)*
- *The Philosophy behind the Agent Runner (Pending)*
